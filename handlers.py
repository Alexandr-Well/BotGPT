...
async def generate_text(msg: Message, state: FSMContext):
    prompt = msg.text
    message = await msg.answer(text.gen_wait)
    res = await utils.generate_text(prompt)
    if not res:
        return await message.edit_text(text.gen_error, reply_markup=kb.iexit_kb)
    await message.edit_text(res[0] + text.text_watermark, disable_web_page_preview=True)
...
async def generate_image(msg: Message, state: FSMContext):
    ...
    await message.answer_photo(photo=img_res[0], caption=text.img_watermark)
